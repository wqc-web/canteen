layui.define(['layer', 'laytpl', 'element'], function(exports) {
  ...
  var Tpl = {
    sideMenu: [
      ...
      'hasItem = typeof item2.list === \'object\' && item2.list.length > 0, ',
      'open = Boolean(item2.open) && hasItem ? \' lau-open\' : \'\'; ',
      'if (icon) { ',
      'if (icon.split(" ").filter(function(x){ return Boolean(x); }).length < 2) { ',
      'icon = \'layui-icon \' + icon; ',
      '} ',
      ...
      'var href = layui.$.trim(item3.href), ',
      'href = href ? \' lau-href="\' + href + \'"\' : \'\', ',
      'title = layui.$.trim(item3.title), ',
      'icon = layui.$.trim(item3.icon); ',
      'if (icon) { ',
      'if (icon.split(" ").filter(function(x){ return Boolean(x); }).length < 2) { ',
      'icon = \'layui-icon \' + icon; ',
      '} ',
      ...
      '{{# } }}',
      '</li>',
      '{{# }); }}','<span id="navBar" class="layui-nav-bar" style="top: 0px; height: 0px; opacity: 0;"></span>',
    ].join(''),
    bodyTab: [
  ...
  var Layout = function () {
    ...
    function traceMenu() {
      ...
      if (menu[0] && !menu.next('.lau-nav-child')[0]) {
        if (menu.hasClass('lau-nav-header')||menu.hasClass('lau-nav-header2')) {
          ...
          if($(pmenu).position()!=null){
            var top = $(pmenu).position().top;
            setTimeout(function navBar() {
              $('#navBar').css('height',$(pmenu).outerHeight());
              $('#navBar').css('opacity','1');
              $('#navBar').css('top',top);
            },300);
          }

          pmenu = menu.parent('dd').parent('dl').parent('dd');
          pmenu[0] && !pmenu.hasClass('lau-open') && pmenu.addClass('lau-open').siblings().removeClass('lau-open');
          if($(pmenu).position()!=null){
            var top = $(pmenu).position().top;
            setTimeout(function navBar() {
      ...
      if(before_el!=null){
        before_el.removeClass("nav-select");
      }
      before_el = $(menu);
      $(menu).addClass("nav-select");
    }
     ...
     * @param el 给选中的菜单添加选中样式
     * @returns {Layout}
     */
    this.go = function (href, title, icon,el) {
      if(before_el!=null){
        before_el.removeClass("nav-select");
      }
      before_el= $(el);
      $(el).addClass("nav-select");
    ...
    $(document).on('click', '*[lau-href]', function () {
      ...
      if (_this.parents('.lau-nav-item')[0]) {
        if (!_this.next('.lau-nav-child')[0]) {
          THIS.go(href, _this.find('cite').text(), _this.find('i').prop('class'),this);
        }
      } else {
        THIS.go(href, _this.attr('lau-title'), _this.attr('lau-icon'),this);
      }
    });
    ...
    $(document).on('click', '.lau-nav-header', function () {
      ...
      setTimeout(function navBar() {
        $('#navBar').css('height',_this.outerHeight());
        $('#navBar').css('opacity','1');
        $('#navBar').css('top',_this.parent().position().top);
      },300);
    });

    $(document).on('click', '.lau-nav-header2', function () {
      var _this = $(this);
      _this.next()[0] ? _this.parent().toggleClass('lau-open').siblings().removeClass('lau-open') : _this.parent().siblings().removeClass('lau-open');
    });
    ...
    $(document).on('mouseenter','.lau-nav-item',function () {
      $('#navBar').css('height',$(this).children("a").outerHeight());
      $('#navBar').css('opacity','1');
      $('#navBar').css('top',$(this).position().top);
    });
